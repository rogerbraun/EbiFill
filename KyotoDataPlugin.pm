...
package Foswiki::Plugins::KyotoDataPlugin;
...
our $SHORTDESCRIPTION = 'The Kyoto Data Plugin.';
...
sub initPlugin {
    ...
    Foswiki::Func::registerRESTHandler( 'filledForms', \&filledFormsHandler );
    Foswiki::Func::registerRESTHandler( 'filledForm', \&filledFormHandler );
    Foswiki::Func::registerRESTHandler( 'fillForms', \&fillFormsHandler );
...
sub extractFormData {
  ...
  foreach my $line(@values) {
    Foswiki::Func::writeDebug($line);
    if($line =~ /name="(.+?)".+?value="(.+)?"/) {
      $ret->{$1} = $2;
    } elsif( $line =~ /%META:TOPICPARENT{name="(.+)"}%/) {
      $ret->{"StudentWikiName"} = $1;
    }
  }
...
sub beforeSaveHandler {
    ...
    Foswiki::Func::writeDebug("Running after save handler...");
    Foswiki::Func::writeDebug("Topic: $topic, Web: $web");
    if($web =~ /KyotoData/) {
      Foswiki::Func::writeDebug("We are in KyotoData web...");
      if($text =~ /%META:FORM{name="Main.StudentKyotoForm"}%/) {
        Foswiki::Func::writeDebug("Form is present...");
        my $data = extractFormData($text);
        sendToHiroshima($data);
        Foswiki::Func::writeDebug("Data sent...");
        my $url = "http://hiroshima.japanologie.kultur.uni-tuebingen.de:10001/";
        $topic =~ /(.*)KyotoData/;
        my $name = $1;
        my $file = "tub_" . $1 . ".pdf";
        ...
        if(index($_[0], $file) != -1) {
          Foswiki::Func::writeDebug("Found link, doing nothing...");
        } else {

          my $restrictions = "   * Set ALLOWTOPICCHANGE = Main.KyotoKommunikationGroup, Main.$name\n   * Set ALLOWTOPICVIEW = Main.KyotoKommunikationGroup, Main.$name\n";
          Foswiki::Func::writeDebug("No link found, adding..."); 
          $_[0] =~ s/%META:TOPICPARENT{name="Main.$name"}%/%META:TOPICPARENT{name="Main.$name"}%\n[[$url$file][PDF zum Ausdrucken]]\n\n$restrictions/;
        }
      }
    }
...
sub sendToHiroshima {
  ...
  $client->POST($url . "/fillForm", "$data",{ "Content-type" => 'application/json'});
}
...
sub generateFiles { 
  my $url = "http://hiroshima.japanologie.kultur.uni-tuebingen.de:10001/";
  my $client = REST::Client->new();
  $client->POST($url . "/generateForms");
}

sub fillFormsHandler {
  my ( $session, $subject, $verb, $response ) = @_;
  my $dh;
  my $file;
  my $folder = "/Users/edv/Sites/foswiki/data/Doshisha/KyotoData";
  my $realret;
  opendir($dh, $folder);

  while($file = readdir($dh)) {
    if($file =~ /KyotoData\.txt$/) {
      $realret = extract_data($folder . "/" . $file);
      sendToHiroshima($realret);
    }
  }
...
sub filledFormHandler {
  my ( $session, $subject, $verb, $response ) = @_;
  my $query = $session->{request};
  my $name = $query->{param}->{files}[0];
  my $files = JSON::decode_json($name);
  my $ret = "";
  my $base = "http://hiroshima.japanologie.kultur.uni-tuebingen.de:10001/";
  Foswiki::Func::writeDebug("Adding one form...");

  my ( $meta, $text) = Foswiki::Func::readTopic("Doshisha/Kommunikation", "StudentData");

  my $ps = "";

  my $olddata  = "";
  if($text =~ /<!-- Auto.*START.*-->((?:.|\n)+)<!-- Auto.*END -->/) {
    Foswiki::Func::writeDebug("Old stuff found\n $1");
    $olddata = $1; 
  }
  $text = "---++ Dateien der Studenten\n\n";
  $text = $text . "<!-- Autogenerated data, do not change START -->\n";
  $ps = "<!-- Autogenerated data, do not change END -->";
  ...
  foreach my $filename (@$files) {
    ...
    my $addition = "| [[$base$filename][$filename]] | [[Doshisha/KyotoData/$name" . "KyotoData]] |\n";
    if(index($olddata, $addition) == -1) {
      Foswiki::Func::writeDebug("Adding $addition");
      $olddata = $olddata . $addition;  
    } else {
      Foswiki::Func::writeDebug("$addition already found, ignoring.");
    }
  }
  
  if(!($olddata =~ /studentData.xls/)) {
    my $addition = "| [[$base" . "studentData.xls][Alle Studenten]] | Gesamtdaten aller Studenten |\n";
    $olddata = $olddata . $addition;
  }

  $text = $text . $olddata . $ps;

  my $hash = {ignorepermissions => 1};
  Foswiki::Func::saveTopic("Doshisha/Kommunikation","StudentData", $meta, $text, $hash);
  return $ret;
}

sub filledFormsHandler {
  my ( $session, $subject, $verb, $response ) = @_;
  my $query = $session->{request};
  my $name = $query->{param}->{files}[0];
  my $files = JSON::decode_json($name);
  ...
  $text = "---++ Dateien der Studenten\n\n";
  $text = $text . "| *Link zur Datei* |\n";
  
  foreach my $filename (@$files) {
    $text = $text . "| [[$base$filename][$filename]] |\n";  
  }
